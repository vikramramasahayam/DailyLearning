...
public class DelayOverflow {
    static void waitForNanoTimeTick() {
        for (final long t0 = System.nanoTime(); t0 == System.nanoTime(););
    }

    void scheduleNow(final ScheduledThreadPoolExecutor pool,
            final Runnable r, final int how) {
        switch (how) {
            case 0:
    ...
    void scheduleAtTheEndOfTime(final ScheduledThreadPoolExecutor pool,
            final Runnable r, final int how) {
        switch (how) {
            case 0:
    ...
    /**
     * Attempts to test exhaustively and deterministically, all 20 possible ways that one task can be scheduled in the maximal distant future, while at the same time an existing tasks's time has already
     * expired.
     */
    void test(final String[] args) throws Throwable {
    ...
    void unexpected(final Throwable t) {
        failed++;
        t.printStackTrace();
    }

    void check(final boolean cond) {
        if (cond)
            pass();
        else fail();
    }

    void equal(final Object x, final Object y) {
    ...
    void instanceMain(final String[] args) throws Throwable {
        try {
            test(args);
        } catch (final Throwable t) {
            unexpected(t);
